...
export function ApplicationScopeSelector({
    ...
    const getHighestImpact = (): ScopeImpactLevel => {
        ...
        if (selectedScopeObjects.some((scope) => scope.impact === "Critical Impact")) return "Critical Impact"
        if (selectedScopeObjects.some((scope) => scope.impact === "High Impact")) return "High Impact"
        if (selectedScopeObjects.some((scope) => scope.impact === "Medium Impact")) return "Medium Impact"
        return "Low Impact"
    }
                                    ...
                                    {roles.map((role) => (
                                        <SelectItem key={role.id} value={role.id}>
                                            <div className="flex items-center">
                                                ...
                                                {role.name}
                                                <span
                                                    className="text-xs text-muted-foreground ml-2">({role.scopes.length} scopes)</span>
                                            </div>
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                    ...
                    {showRoleWarning && (
                        <div className="flex items-center p-3 border rounded-md bg-amber-50 text-amber-800 gap-2">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs">Applying a role template will override any scopes you've already
                                    selected.</p>
                            </div>
                            <Button
                                variant="outline"
                                                    ...
                                                    <div key={scope.id}
                                                        ...
                                                        <div>
                                                            ...
                                                            >
                                                                {scope.name}
                                                                <Badge
                                                                    className={`${getImpactLevelColor(scope.impact)} border ml-2 text-xs`}>
                                                                    {scope.impact}
                                                                </Badge>
                                                            </Label>
                                    ...
                                    {["critical", "high", "medium", "low"].map((impact) => (
                                        <div key={impact} className="mb-4">
                                            <h4 className={`text-sm font-medium mb-2 flex items-center`}>
                                                <Badge
                                                    className={`${getImpactLevelColor(impact as ScopeImpactLevel)} border mr-2`}>
